...
void checkInference()
{
    ...
    Short_t len_list[randSeeds.size()];
    ...
    for(int k = 0; k < randSeeds.size(); ++k)
    {
        std::cout << len_list[k] << ", ";
    }
    std::cout << "\n";
...
/*
float best_CalcVals[6] = {0.370105,std::log10(0.0203667),std::log10(0.0384742),std::pow(std::sin(0.884828),2),1.21022,0.00253644};
    setCalcVals(calc_best,best_CalcVals);
    ...
    double currChi = multiExp.ChiSq(calc_best);
    printCalc(calc_best);
    double chi2All = mfitter.Fit(calc_best, MinuitFitter::kQuiet)->EvalMetricVal();
    printCalc(calc_best);
    std::cout << currChi << " " <<chi2All << "\n";
